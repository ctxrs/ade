...
describe("sessionMeasurementPolicy", () => {
  it("routes complex inline-code markdown to rendered measurement", () => {
    const markdown = [
      ...
      "",
      "The exact commit should stay aligned with the green Buildkite run.",
    ].join("\n");
  ...
  it("routes long inline-code assistant rows to rendered measurement", () => {
    const content =
      "The fix is on `origin/main` now at `9771951d0`, and Buildkite picked it up immediately as `ctx-main #42`. I’m waiting for that exact commit to go green before I fire the new canary release.";
