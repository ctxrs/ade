...
test("macOS AVF exhaustive workflow stays manual+scheduled and targets the dedicated AVF runner label", () => {
  const text = fs.readFileSync(workflowPath, "utf8");
  assert.match(text, /workflow_dispatch:/);
  assert.match(text, /schedule:/);
  assert.match(text, /runs-on:\s*\$\{\{\s*vars\.AVF_RUNNER_LABEL\s*\|\|\s*'macos-15'\s*\}\}/);
