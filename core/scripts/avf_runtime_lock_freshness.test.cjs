...
test("validateAvfRuntimeFreshness allows a thin manifest when managed AVF metadata is present", () => {
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "ctx-avf-runtime-freshness-thin-"));
  const runtimeDir = path.join(tmpRoot, "runtime");
  const bundleDir = path.join(tmpRoot, "bundles");
  const version = "ubuntu-noble-arm64-thin12345678";
  fs.mkdirSync(runtimeDir, { recursive: true });
  fs.mkdirSync(bundleDir, { recursive: true });
  fs.writeFileSync(
    path.join(runtimeDir, "version.txt"),
