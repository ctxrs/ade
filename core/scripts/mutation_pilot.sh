...
mutations=(
$'store_fault_injection_no_error\tstore\tcrates/ctx-store/src/fault_injection.rs\tErr(anyhow::anyhow!("fault injection: {point}"))\tOk(())\tnode scripts/run_with_ctx_cache_env.cjs --mode workspace --cwd . -- cargo test -p ctx-store --features fault_injection'
$'http_fault_injection_no_error\thttp\tcrates/ctx-http/src/fault_injection.rs\tErr(anyhow::anyhow!("fault injection: {point}"))\tOk(())\tnode scripts/run_with_ctx_cache_env.cjs --mode workspace --cwd . -- cargo test -p ctx-http --features fault_injection --test fault_matrix'
$'store_snapshot_keeps_assistant_partial\tstore\tcrates/ctx-store/src/store/mod.rs\tturn.assistant_partial = None;\tturn.assistant_partial = turn.assistant_partial.clone();\tnode scripts/run_with_ctx_cache_env.cjs --mode workspace --cwd . -- cargo test -p ctx-store session_head_snapshot_excludes_assistant_partials'
$'store_snapshot_keeps_assistant_chunk_events\tstore\tcrates/ctx-store/src/store/mod.rs\tSessionEventType::AssistantChunk | SessionEventType::ThoughtChunk\tSessionEventType::Notice | SessionEventType::ThoughtChunk\tnode scripts/run_with_ctx_cache_env.cjs --mode workspace --cwd . -- cargo test -p ctx-store session_head_snapshot_excludes_assistant_partials'
$'updater_non_linux_in_place_allowed\tupdater-http\tcrates/ctx-http/src/api/updates.rs\tif !platform_key.starts_with("linux-") {\tif false {\tnode scripts/run_with_ctx_cache_env.cjs --mode workspace --cwd . -- cargo test -p ctx-http --lib in_place_update_capability_is_false_for_non_linux_platforms'
$'updater_unsupported_platform_allows_update\tupdater-http\tcrates/ctx-http/src/api/updates.rs\tif !platform_supported {\tif false {\tnode scripts/run_with_ctx_cache_env.cjs --mode workspace --cwd . -- cargo test -p ctx-http --lib in_place_update_capability_is_false_when_platform_is_not_supported'
