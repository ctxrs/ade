...
const sessionControlHandleApiPaths = new Set([
...
const sessionTitleModelModeHandleApiPaths = new Set([
  "core/crates/ctx-http/src/api/sessions/titles_and_modes.rs",
  "core/crates/ctx-http/src/api/sessions/titles_and_modes/title.rs",
  "core/crates/ctx-http/src/api/sessions/titles_and_modes/model.rs",
...
const sessionSubagentMcpControlHandlerNamesByPath = new Map([
  [
    "core/crates/ctx-http/src/api/sessions/subagents/init.rs",
    ["mcp_spawn_agent"],
  ],
  [
    "core/crates/ctx-http/src/api/sessions/subagents/handlers.rs",
    ["mcp_send_input", "mcp_archive_agent", "mcp_interrupt_agent"],
  ],
]);
...
const sessionSubagentMcpControlDaemonImplementationPaths = new Set([
  "core/crates/ctx-daemon/src/daemon/sessions/subagents_route.rs",
  "core/crates/ctx-daemon/src/daemon/sessions/handle.rs",
]);
...
const sessionTitleModelModeDaemonImplementationPaths = new Set([
  "core/crates/ctx-daemon/src/daemon/sessions/title_model_mode_route.rs",
  "core/crates/ctx-daemon/src/daemon/sessions/model_target_bridge.rs",
  "core/crates/ctx-daemon/src/daemon/sessions/model_switch.rs",
]);
...
const TITLE_GENERATION_API_ROUTE_CONTRACT_PATTERNS = [
  {
    name: "title-generation API imports daemon beyond title-generation local handle",
    regex: /a^/,
    contentRegex: /\bctx_daemon\b/gm,
    allowContentRegex: /\buse\s+ctx_daemon::daemon::TitleGenerationLocalHandle\s*;/gm,
  },
];
...
function scanDaemonHandleConstructionRatchet({ filePath, contents }) {
  ...
  if (filePath === "core/crates/ctx-daemon/src/daemon/handle.rs") {
    const routeFactoryRegex =
      /\b(?:pub(?:\s*\([^)]*\))?\s+)?fn\s+([a-z_][A-Za-z0-9_]*)\s*\([^)]*\)\s*->\s*(?!Self\b)[^{;]*(?:[A-Za-z][A-Za-z0-9_]*(?:Handle|Lookup|Host|Effects|Runtime)|DaemonRouteHandles)\b/gsu;
    for (
      let match = routeFactoryRegex.exec(contents);
      match;
      match = routeFactoryRegex.exec(contents)
    ) {
      const lineIndex = lineForOffset(match.index);
      const key = `${lineIndex + 1}:daemon handle route factory method`;
      if (violationKeys.has(key)) {
        continue;
      }
      violationKeys.add(key);
      violations.push({
        filePath,
        line: lineIndex + 1,
        name: "daemon handle route factory method",
        text: lines[lineIndex]?.trim() || match[0].trim().replace(/\s+/g, " "),
      });
    }
  }
  return violations;
...
function scanTaskRouteHandleBroadStructFields({
  filePath,
  contents,
  handleNames,
  violationName,
...
function scanTaskAdmissionHandleFieldRatchet({ filePath, contents }) {
  if (!taskRouteHandleFieldRatchetPaths.has(filePath)) {
    return [];
  }
  return scanTaskRouteHandleBroadStructFields({
    filePath,
    contents,
    handleNames: ["TaskCreationHandle", "TaskSessionAdmissionHandle"],
    violationName: "task admission capability stores broad handle or daemon state",
  });
}
...
function scanTaskLifecycleHandleFieldRatchet({ filePath, contents }) {
  ...
  const violations = scanTaskRouteHandleBroadStructFields({
    filePath,
    contents,
    handleNames: ["TaskLifecycleHandle"],
    violationName: "task lifecycle capability stores broad handle or daemon state",
  });
  const lines = contents.split(/\r?\n/u);
...
function scanTaskReadMetadataHandleFieldRatchet({ filePath, contents }) {
  ...
  const violations = scanTaskRouteHandleBroadStructFields({
    ...
    contents,
    handleNames,
    violationName: "task read metadata capability stores broad handle or daemon state",
  });
  const lines = contents.split(/\r?\n/u);
...
function scanSessionControlHandleRatchet({ filePath, contents }) {
  const violations = [];
  const lines = contents.split(/\r?\n/u);
  if (sessionControlHandleApiPaths.has(filePath)) {
    const sessionsHandleRegex = /\bSessionsHandle\b|State\s*<\s*SessionsHandle\s*>/gu;
    for (
      let match = sessionsHandleRegex.exec(contents);
      match;
      ...
      violations.push({
        filePath,
        line,
        name: "session control route extracts broad sessions handle",
        text: lines[line - 1]?.trim() ?? match[0],
      });
    }
  }
  if (filePath === "core/crates/ctx-http/src/api/router.rs") {
    const routeWiringRegex =
      /\bsession_control\s*:\s*handle\.sessions\s*\(\s*\)|\bSessionControlHandle\s*,\s*sessions\s*;/gu;
    for (
      let match = routeWiringRegex.exec(contents);
      match;
      match = routeWiringRegex.exec(contents)
      ...
      violations.push({
        filePath,
        line,
        name: "session control route exposes broad sessions handle",
        text: lines[line - 1]?.trim() ?? match[0],
      });
    }
  }
...
function scanSessionFileCompletionsHandleRatchet({ filePath, contents }) {
  const violations = [];
  const lines = contents.split(/\r?\n/u);
  if (sessionFileCompletionsHandleApiPaths.has(filePath)) {
    const sessionsHandleRegex = /\bSessionsHandle\b|State\s*<\s*SessionsHandle\s*>/gu;
    for (
      let match = sessionsHandleRegex.exec(contents);
      match;
      ...
      violations.push({
        filePath,
        line,
        name: "session file-completions route extracts broad sessions handle",
        text: lines[line - 1]?.trim() ?? match[0],
      });
    }
  }
  if (filePath === "core/crates/ctx-http/src/api/router.rs") {
    const routeWiringRegex =
      /\bsession_file_completions\s*:\s*handle\.sessions\s*\(\s*\)|\bSessionFileCompletionsHandle\s*,\s*sessions\s*;/gu;
    for (
      let match = routeWiringRegex.exec(contents);
      match;
      match = routeWiringRegex.exec(contents)
      ...
      violations.push({
        filePath,
        line,
        name: "session file-completions route exposes broad sessions handle",
        text: lines[line - 1]?.trim() ?? match[0],
      });
    }
  }
...
function scanSessionTitleModelModeHandleRatchet({ filePath, contents }) {
  const violations = [];
  const lines = contents.split(/\r?\n/u);
  if (sessionTitleModelModeHandleApiPaths.has(filePath)) {
    const sessionsHandleRegex = /\bSessionsHandle\b|State\s*<\s*SessionsHandle\s*>/gu;
    for (
      let match = sessionsHandleRegex.exec(contents);
      match;
      ...
      violations.push({
        filePath,
        line,
        name: "session title/model/mode route extracts broad sessions handle",
        text: lines[line - 1]?.trim() ?? match[0],
      });
    }
  }
  if (filePath === "core/crates/ctx-http/src/api/router.rs") {
    const routeWiringRegex =
      /\bsession_title_model_mode\s*:\s*handle\.sessions\s*\(\s*\)|\bSessionTitleModelModeHandle\s*,\s*sessions\s*;/gu;
    for (
      let match = routeWiringRegex.exec(contents);
      match;
      match = routeWiringRegex.exec(contents)
      ...
      violations.push({
        filePath,
        line,
        name: "session title/model/mode route exposes broad sessions handle",
        text: lines[line - 1]?.trim() ?? match[0],
      });
    }
  }
...
function scanSessionSubagentMcpControlHandleRatchet({ filePath, contents }) {
  const violations = [];
  const lines = contents.split(/\r?\n/u);
  const handlerNames = sessionSubagentMcpControlHandlerNamesByPath.get(filePath) ?? [];
  if (handlerNames.length > 0) {
    const sessionsHandleRegex = /\bSessionsHandle\b|State\s*<\s*SessionsHandle\s*>/gu;
    for (const fnName of handlerNames) {
      const block = rustFunctionBlockForName({ contents, fnName });
      if (!block) {
        continue;
      }
      ...
      for (
        ...
        violations.push({
          filePath,
          line,
          name: "session subagent MCP control route extracts broad sessions handle",
          text: lines[line - 1]?.trim() ?? match[0],
        });
      }
    }
  ...
  if (filePath === "core/crates/ctx-http/src/api/router.rs") {
    const routeWiringRegex =
      /\bsession_subagent_mcp_control\s*:\s*handle\.sessions\s*\(\s*\)|\bSessionSubagentMcpControlHandle\s*,\s*sessions\s*;/gu;
    for (
      let match = routeWiringRegex.exec(contents);
      match;
      match = routeWiringRegex.exec(contents)
      ...
      violations.push({
        filePath,
        line,
        name: "session subagent MCP control route exposes broad sessions handle",
        text: lines[line - 1]?.trim() ?? match[0],
      });
    }
  }
...
function scanFinalSessionRouteCapabilityRatchet({ filePath, contents }) {
  const violations = [];
  const lines = contents.split(/\r?\n/u);
  ...
  if (
    ...
    for (
      ...
      violations.push({
        filePath,
        line,
        name: "final session route extracts broad sessions handle",
        text: lines[line - 1]?.trim() ?? match[0],
      });
    }
  }
  ...
  if (filePath === "core/crates/ctx-http/src/api/router.rs") {
    const routeWiringRegex =
      /\bsessions\s*:\s*SessionsHandle\b|\bsessions\s*:\s*handle\.sessions\s*\(\s*\)|\bSessionsHandle\s*,\s*sessions\s*;|\bhandle\.sessions\s*\(\s*\)/gu;
    for (
      let match = routeWiringRegex.exec(contents);
      match;
      match = routeWiringRegex.exec(contents)
      ...
      violations.push({
        filePath,
        line,
        name: "final session route exposes broad sessions handle in router state",
        text: lines[line - 1]?.trim() ?? match[0],
      });
    }
  }
  ...
  if (
    ...
    filePath === "core/crates/ctx-daemon/src/daemon/sessions/demo_route.rs" ||
    filePath === "core/crates/ctx-daemon/src/daemon/sessions/demo_seed.rs"
  ) {
    const staleFacadeRegex =
      /\b(?:title_generation_local_status|start_title_generation_local_install|seed_demo_transcript_for_route|seed_demo_transcript)\b/gu;
    for (const impl of rustImplBlocksForType({ contents, typeName: "SessionsHandle" })) {
      staleFacadeRegex.lastIndex = 0;
      for (
        let match = staleFacadeRegex.exec(impl.text);
        match;
        match = staleFacadeRegex.exec(impl.text)
      ) {
        ...
        violations.push({
          filePath,
          line,
          name: "final session route facade remains on broad sessions handle",
          text: lines[line - 1]?.trim() ?? match[0],
        });
      }
    }
  ...
  const guardedHandleTypesByPath = new Map([
    [
      "core/crates/ctx-daemon/src/daemon/sessions/title_generation.rs",
      ["TitleGenerationLocalHandle", "TitleGenerationLocalInstallEffect"],
    ],
    [
      "core/crates/ctx-daemon/src/daemon/sessions/demo_seed.rs",
      ["DemoSeedTranscriptHandle", "DemoSeedTranscriptRefreshHost"],
    ],
  ]);
  const guardedHandleTypes = guardedHandleTypesByPath.get(filePath) ?? [];
  if (guardedHandleTypes.length === 0) {
    return violations;
  }

  const broadSeamRegex =
    /\b(?:DaemonState|DaemonHandle|SessionsHandle|ManagedInstallHost|AppState)\b|\bArc\s*<\s*DaemonState\s*>/gu;
  const genericEscapeFieldRegex =
    /^\s*(?:pub(?:\s*\([^)]*\))?\s+)?(?:daemon|state|sessions|handle)\s*:/gmu;
  for (const typeName of guardedHandleTypes) {
    const structBlock = rustStructBlockForType({ contents, typeName });
    if (structBlock) {
      broadSeamRegex.lastIndex = 0;
      for (
        let match = broadSeamRegex.exec(structBlock.text);
        match;
        match = broadSeamRegex.exec(structBlock.text)
      ) {
        const offset = structBlock.index + match.index;
        const line = contents.slice(0, offset).split(/\r?\n/u).length;
        violations.push({
          filePath,
          line,
          name: "final session route capability stores broad daemon seam",
          text: lines[line - 1]?.trim() ?? match[0],
        });
      }
      ...
      for (
        let match = genericEscapeFieldRegex.exec(structBlock.text);
        match;
        match = genericEscapeFieldRegex.exec(structBlock.text)
      ) {
        const offset = structBlock.index + match.index;
        const line = contents.slice(0, offset).split(/\r?\n/u).length;
        violations.push({
          filePath,
          line,
          name: "final session route capability exposes generic full-state field",
          text: lines[line - 1]?.trim() ?? match[0],
        });
      }
    }
    ...
    for (const impl of rustImplBlocksForType({ contents, typeName })) {
      ...
      for (
        ...
        violations.push({
          filePath,
          line,
          name: "final session route capability method uses broad daemon seam",
          text: lines[line - 1]?.trim() ?? match[0],
        });
      }
    }
...
function scanSessionControlDaemonImplementationRatchet({ filePath, contents }) {
  if (!sessionControlDaemonImplementationPaths.has(filePath)) {
    return [];
  }
  const violations = [];
  const lines = contents.split(/\r?\n/u);
  const checks = [
    {
      name: "session control daemon implementation uses broad daemon handle",
      regex: /\bDaemonHandle\b/gu,
    },
    {
      name: "session control daemon implementation uses broad session handle",
      regex: /\bSessionsHandle\b/gu,
    },
    {
  ...
  for (const typeName of ["SessionControlHandle", "SessionFileCompletionsHandle"]) {
    for (const impl of rustImplBlocksForType({ contents, typeName })) {
      const implChecks = [
        {
          name: "session control capability impl uses broad daemon handle",
          regex: /\bDaemonHandle\b/gu,
        },
        {
          name: "session control capability impl uses broad session handle",
          regex: /\bSessionsHandle\b/gu,
        },
        {
...
function scanSessionFileCompletionsDaemonSeamRatchet({ filePath, contents }) {
  if (
  ...
  const violations = [];
  const lines = contents.split(/\r?\n/u);
  if (filePath === "core/crates/ctx-daemon/src/daemon/sessions/workspace_bridge.rs") {
    const staleHelperRegex = /\bcomplete_files_for_session\s*\(/gu;
    for (
      let match = staleHelperRegex.exec(contents);
      match;
      match = staleHelperRegex.exec(contents)
    ) {
      const line = contents.slice(0, match.index).split(/\r?\n/u).length;
      violations.push({
        filePath,
        line,
        name: "session file-completions broad session helper remains",
        text: lines[line - 1]?.trim() ?? match[0],
      });
    }
  }
  if (isDaemonRouteAssemblyPath(filePath)) {
    const block = rustFunctionBlockForName({ contents, fnName: "session_file_completions" });
    if (block) {
      const hiddenStateRegex =
        /\blet\s+state\s*=\s*Arc::clone\s*\(\s*&self\.state\s*\)|\bcomplete_files_for_session\s*\(/gu;
      for (
        ...
        violations.push({
          filePath,
          line,
          name: "session file-completions handle hides daemon state seam",
          text: lines[line - 1]?.trim() ?? match[0],
        });
      }
    }
...
function scanSessionTitleModelModeDaemonImplementationRatchet({ filePath, contents }) {
  if (!sessionTitleModelModeDaemonImplementationPaths.has(filePath)) {
    return [];
  }
  const violations = [];
  const lines = contents.split(/\r?\n/u);
  const checks = [
    {
      name: "session title/model/mode daemon implementation uses broad daemon handle",
      regex: /\bDaemonHandle\b/gu,
    },
    {
      name: "session title/model/mode daemon implementation uses broad session handle",
      regex: /\bSessionsHandle\b/gu,
    },
    {
      name: "session title/model/mode daemon implementation accepts daemon state",
      regex: /\bDaemonState\b|\bArc\s*<\s*DaemonState\s*>/gu,
    },
  ];
  for (const check of checks) {
...
function scanSessionSubagentMcpControlDaemonImplementationRatchet({ filePath, contents }) {
  if (!sessionSubagentMcpControlDaemonImplementationPaths.has(filePath)) {
    return [];
  }
  const violations = [];
  const lines = contents.split(/\r?\n/u);
  ...
  if (filePath === "core/crates/ctx-daemon/src/daemon/sessions/subagents_route.rs") {
    const controlFacadeRegex =
      /\b(?:spawn_agent_for_mcp_route|send_input_for_mcp_route|archive_agent_for_mcp_route|interrupt_agent_for_mcp_route)\b/gu;
    for (const impl of rustImplBlocksForType({ contents, typeName: "SessionsHandle" })) {
      controlFacadeRegex.lastIndex = 0;
      for (
        let match = controlFacadeRegex.exec(impl.text);
        match;
        match = controlFacadeRegex.exec(impl.text)
      ) {
        const offset = impl.index + match.index;
        const line = contents.slice(0, offset).split(/\r?\n/u).length;
        violations.push({
          filePath,
          line,
          name: "session subagent MCP control facade remains on broad sessions handle",
          text: lines[line - 1]?.trim() ?? match[0],
        });
      }
    }
    ...
    for (const impl of rustImplBlocksForType({
      contents,
      typeName: "SessionSubagentMcpControlHandle",
    })) {
      broadSeamRegex.lastIndex = 0;
      for (
        let match = broadSeamRegex.exec(impl.text);
        ...
        violations.push({
          filePath,
          line,
          name: "session subagent MCP control daemon implementation uses broad seam",
          text: lines[line - 1]?.trim() ?? match[0],
        });
      }
    }
  ...
  if (filePath === "core/crates/ctx-daemon/src/daemon/sessions/handle.rs") {
    const requestMethodRegex =
      /\bpub\s+(?:async\s+)?fn\s+(?:spawn_agent|send_input|archive_agent|interrupt_agent)\s*\(/gu;
    for (const impl of rustImplBlocksForType({ contents, typeName: "SessionsHandle" })) {
      requestMethodRegex.lastIndex = 0;
      for (
        let match = requestMethodRegex.exec(impl.text);
        ...
        violations.push({
          filePath,
          line,
          name: "session subagent MCP control request method remains on broad sessions handle",
          text: lines[line - 1]?.trim() ?? match[0],
        });
      }
    }
  ...
  for (const typeName of [...controlTypeNames, ...fileCompletionTypeNames]) {
    ...
    for (
      ...
      violations.push({
        filePath,
        line,
        name: "session control capability stores broad handle or daemon state",
        text: lines[line - 1]?.trim() ?? broad[0],
      });
    }
...
function scanSessionTitleModelModeHandleFieldRatchet({ filePath, contents }) {
  ...
  const violations = [];
  const lines = contents.split(/\r?\n/u);
  const structBlock = rustStructBlockForType({
    contents,
    typeName: "SessionTitleModelModeHandle",
  });
  if (!structBlock) {
    return violations;
  }
  const broadFieldRegex =
    /\b(?:SessionsHandle|WorkspacesHandle|ProvidersHandle|DaemonHandle|DaemonState|StoreManager|WorkspaceRuntime|ProviderWorkspaceLaunchRuntime)\b|\bArc\s*<\s*DaemonState\s*>/gu;
  for (
    let broad = broadFieldRegex.exec(structBlock.text);
    broad;
    broad = broadFieldRegex.exec(structBlock.text)
    ...
    violations.push({
      filePath,
      line,
      name: "session title/model/mode capability stores broad handle or runtime bag",
      text: lines[line - 1]?.trim() ?? broad[0],
    });
  }
  ...
  for (
    ...
    violations.push({
      filePath,
      line,
      name: "session title/model/mode capability exposes generic full-state field",
      text: lines[line - 1]?.trim() ?? generic[0],
    });
  }
  return violations;
...
function scanSessionSubagentMcpControlHandleFieldRatchet({ filePath, contents }) {
  if (filePath !== "core/crates/ctx-daemon/src/daemon/sessions/subagents/mcp_control.rs") {
    return [];
  }
  const violations = [];
  const lines = contents.split(/\r?\n/u);
  ...
  for (const typeName of [
    ...
    "SessionSubagentMcpControlHandleParts",
  ]) {
    const handleStruct = rustStructBlockForType({ contents, typeName });
    if (!handleStruct) {
      continue;
    }
    ...
    for (
      ...
      violations.push({
        filePath,
        line,
        name: "session subagent MCP control capability stores broad handle or runtime bag",
        text: lines[line - 1]?.trim() ?? broad[0],
      });
    }
    ...
    for (
      ...
      violations.push({
        filePath,
        line,
        name: "session subagent MCP control capability exposes generic full-state field",
        text: lines[line - 1]?.trim() ?? generic[0],
      });
    }
  }
  ...
  for (const impl of rustImplBlocksForType({
    ...
    for (
      ...
      violations.push({
        filePath,
        line,
        name: "session subagent MCP control capability exposes store escape hatch",
        text: lines[line - 1]?.trim() ?? escape[0],
      });
    }
  }
...
function scanSessionTitleModelModeTitleImplementationRatchet({ filePath, contents }) {
  ...
  const lines = contents.split(/\r?\n/u);
  const broadSeamRegex =
    /\bDaemonState\b|\bArc\s*<\s*DaemonState\s*>|\bSessionsHandle\b|\bDaemonHandle\b/gu;

  let blocks = [];
  if (filePath === "core/crates/ctx-daemon/src/daemon/sessions/handle.rs") {
    blocks = rustImplBlocksForType({
      contents,
      typeName: "SessionTitleModelModeHandle",
    });
  } else if (filePath === "core/crates/ctx-daemon/src/daemon/sessions/title_generation.rs") {
    blocks = [
      rustFunctionBlockForName({
        contents,
        fnName: "maybe_generate_session_title_with_handle",
      }),
    ].filter(Boolean);
  } else if (
    filePath ===
    ...
    blocks = [
      rustFunctionBlockForName({
        contents,
        fnName: "apply_session_title_update_with_handle",
      }),
    ].filter(Boolean);
  }
  ...
  for (const block of blocks) {
    broadSeamRegex.lastIndex = 0;
    for (
      let broad = broadSeamRegex.exec(block.text);
      broad;
      broad = broadSeamRegex.exec(block.text)
    ) {
      const offset = block.index + broad.index;
      const line = contents.slice(0, offset).split(/\r?\n/u).length;
      violations.push({
        filePath,
        line,
        name: "session title/model/mode title implementation uses broad daemon seam",
        text: lines[line - 1]?.trim() ?? broad[0],
      });
    }
  }
...
function scanSessionTitleModelModeAssemblyRatchet({ filePath, contents }) {
  ...
  const violations = [];
  const lines = contents.split(/\r?\n/u);
  const block = rustFunctionBlockForName({
    contents,
    fnName: "session_title_model_mode",
  });
  if (!block) {
    return violations;
  }
  const hiddenDaemonStateModelCatalogRegex =
    /\bmodel_catalog_loader\s*=\s*Arc::new\b|\blet\s+state\s*=\s*Arc::clone\s*\(\s*&self\.state\s*\)|\bload_provider_model_catalog_for_execution_environment\s*\(\s*&state\b/gu;
  for (
    let hidden = hiddenDaemonStateModelCatalogRegex.exec(block.text);
    hidden;
    hidden = hiddenDaemonStateModelCatalogRegex.exec(block.text)
  ) {
    const offset = block.index + hidden.index;
    const line = contents.slice(0, offset).split(/\r?\n/u).length;
    violations.push({
      filePath,
      line,
      name: "session title/model/mode assembly hides daemon state model-catalog seam",
      text: lines[line - 1]?.trim() ?? hidden[0],
    });
  }
  return violations;
...
function scanWorkspaceRouteBuilderStateAssemblyRatchet({ filePath, contents }) {
  const violations = [];
  const lines = contents.split(/\r?\n/u);
  if (
    filePath.startsWith(daemonRouteBuildersRelativeRoot) &&
    ...
    const childWorkspaceDepsRegex = /\bworkspace_route_deps\s*\(/gu;
    for (
      let match = childWorkspaceDepsRegex.exec(contents);
      match;
      match = childWorkspaceDepsRegex.exec(contents)
    ) {
      const line = contents.slice(0, match.index).split(/\r?\n/u).length;
      violations.push({
        filePath,
        line,
        name: "workspace route child builder reconstructs workspace deps",
        text: lines[line - 1]?.trim() ?? match[0],
      });
    }
  }
  if (!workspaceRouteBuilderAssemblyPaths.has(filePath)) {
    return violations;
  }
  const checks = [
    {
      name: "workspace route builder uses broad daemon state",
      regex: /\bDaemonState\b|\bArc\s*<\s*DaemonState\s*>/gu,
    },
    {
      name: "workspace route builder accesses broad route-builder state",
      regex: /\bself\s*\.\s*state\b/gu,
    },
  ];
  for (const check of checks) {
...
function scanRepo() {
  ...
  if (fs.existsSync(daemonTaskRouteHandlesPath)) {
    const relativePath = repoRelative(daemonTaskRouteHandlesPath);
    const contents = fs.readFileSync(daemonTaskRouteHandlesPath, "utf8");
    violations.push(
      ...scanTaskAdmissionHandleFieldRatchet({
        filePath: relativePath,
        contents,
