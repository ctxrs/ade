...
test("merge queue route handle split module has a line-cap ratchet", () => {
  const rootDir = makeRoot();
  const capped = RATCHETED_FILE_LIMITS.find((entry) =>
    entry.path === "core/crates/ctx-daemon/src/daemon/merge_queue_route_handles.rs"
  );
  assert.ok(capped, "expected merge_queue_route_handles.rs to have a line cap");
  writeFile(rootDir, capped.path, lines(capped.limit + 1));
...
test("final tail route handle split modules have line-cap ratchets", () => {
  const rootDir = makeRoot();
  const cappedPaths = [
    "core/crates/ctx-daemon/src/daemon/settings_route_handles.rs",
