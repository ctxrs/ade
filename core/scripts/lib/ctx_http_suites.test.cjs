...
const nonDuplicateAllSuiteNames = CTX_HTTP_SUITES
...
test("ctx-http suite command builder expands base and meta suites predictably", () => {
  ...
  assert.deepEqual(
    ...
    nonDuplicateAllSuiteNames.map((suiteName) => `${CTX_HTTP_BAZEL_PACKAGE}:${suiteName}`),
  );
  assert.equal(new Set(getCtxHttpSuiteTargets("all")).size, nonDuplicateAllSuiteNames.length);
  assert.equal(getCtxHttpSuiteTargets("all").includes(`${CTX_HTTP_BAZEL_PACKAGE}:unit-tests-api`), false);
  assert.equal(getCtxHttpSuiteTargets("all").includes(`${CTX_HTTP_BAZEL_PACKAGE}:base`), true);
