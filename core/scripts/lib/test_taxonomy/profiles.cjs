...
const PROFILES = [
  ...
  },
  {
    ...
    currentExecution: "The listed commands run nightly breadth checks; the checked-in ctx-nightly pipeline is a disabled scaffold.",
    expansionRules: [
      "Nightly is allowed to exercise live-provider and wide-matrix flows that are intentionally excluded from the inner loop.",
    ],
  },
  {
    id: "nightly-scaffold-disabled",
    title: "Nightly Scaffold Disabled",
    purpose: "Checked-in placeholder profile for the currently disabled nightly pipeline.",
    selector: {
      includeEntryIds: [
        "repo-contracts.nightly-scaffold-disabled",
      ],
      includeSurfaces: ["contract"],
      includeWorlds: ["hermetic"],
      includeCosts: ["tiny"],
      includeStabilities: ["experimental"],
      includeExecutions: ["script-local"],
      excludeRequirements: ["mac", "single-mac", "long-running"],
    },
    currentCommands: [
      "Buildkite pipeline: ctx-nightly",
      "pnpm -C core testing:profile:run --profile nightly-scaffold-disabled",
    ],
    pipelines: ["ctx-nightly"],
    remoteStrategy: "This script-local profile selects the disabled nightly scaffold contract.",
    currentExecution: "Runs the checked-in disabled scaffold message through a taxonomy-owned wrapper so pipeline/profile reporting stays honest.",
    expansionRules: [
      "This profile runs the scaffold message, not the nightly-breadth checks.",
    ],
  },
];
