...
const REQUIRED_TABLES = Object.freeze([
  ...
  "ctx.telemetry_event",
  "ctx.analytics_contact_install_attribution",
...
const REQUIRED_INDEXES = Object.freeze([
  ...
  "billing_spend_limits_active_period_idx",
  ...
  "analytics_contact_install_attribution_unique_idx",
  "analytics_contact_install_attribution_install_idx",
  "analytics_contact_install_attribution_contact_idx",
  "analytics_contact_install_attribution_attributed_at_idx",
  "telemetry_event_agent_real_events_idx",
  "telemetry_event_agent_install_event_idx",
  "telemetry_event_agent_event_name_time_idx",
  ...
  "mobile_tunnel_relay_node_region_health_idx",
  "mobile_tunnel_user_active_idx",
  "mobile_tunnel_binding_active_idx",
  "mobile_tunnel_grant_active_idx",
  "mobile_tunnel_event_tunnel_observed_idx",
  "mobile_tunnel_event_user_observed_idx",
...
const REQUIRED_SQL_PATTERNS = Object.freeze([
  ...
  {
    label: "relay spend limit schema exposes runtime enforcement columns",
    pattern: /\badd\s+column\s+if\s+not\s+exists\s+hard_limit_cents\b[\s\S]*\bbilling_spend_limits_active_period_idx\b/i,
  },
  {
    label: "mobile tunnel runtime role cannot mutate grant rows",
    pattern: /\brevoke\s+insert\s*,\s*update\s+on\s+ctx\s*\.\s*mobile_tunnel_grants\s+from\s+ctx_mobile_tunnel\b/i,
  },
  {
    label: "relay pricing preserves per-1k token precision",
    pattern: /\badd\s+column\s+if\s+not\s+exists\s+input_microusd_per_1k_tokens\s+bigint\b[\s\S]*\badd\s+column\s+if\s+not\s+exists\s+output_microusd_per_1k_tokens\s+bigint\b/i,
  },
]);
