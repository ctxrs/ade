...
test("release preflight provisions the native sandbox CLI before the linux-arm critical lane", () => {
  ...
  assert.match(
    text,
    /name:\s+Install Linux sandbox runtime CLI[\s\S]*install_linux_sandbox_runtime_cli\.sh --require-reachable[\s\S]*CTX_HARNESS_SANDBOX_CLI_PATH[\s\S]*CONTAINERD_ADDRESS=\/run\/containerd\/containerd\.sock[\s\S]*CONTAINERD_NAMESPACE=default[\s\S]*name:\s+Run linux-arm critical provider lane/s,
  );
});
