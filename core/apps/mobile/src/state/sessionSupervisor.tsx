...
export class SessionSupervisor {
  ...
  async loadMoreTurns(sessionId: string) {
    ...
    try {
      const page = await getSessionHistory(this.conn, sessionId, beforeSeq, TURN_PAGE_LIMIT);
  ...
  async loadTurnTools(sessionId: string, turnId: string) {
    ...
    this.publish();
    try {
      const tools = await listTurnTools(this.conn, sessionId, turnId);
...
export function SessionSupervisorProvider({ children }: { children: React.ReactNode }) {
  ...
  } else if (!supRef.current || lastKeyRef.current !== key) {
    supRef.current = new SessionSupervisor(config);
...
export function useOpenSession(sessionId: string, opts?: OpenOptions) {
  ...
  const stableOpts = useMemo(
    () => ({ watchDiff: opts?.watchDiff ?? false, force: opts?.force ?? false }),
    [opts?.watchDiff, opts?.force],
  );
  useEffect(() => {
    if (!sessionId) return;
...
const extractToolStatus = (event: SessionEvent): string | null => {
  const payload = event.payload_json ?? {};
  const status = payload?.status ?? payload?.tool_result?.status ?? payload?.tool_call?.status;
  if (typeof status === "string" && status.trim()) return status;
  if (event.event_type === "tool_call") return "running";
  if (event.event_type === "tool_call_update") return "running";
...
const deriveTurnStatusFromEvent = (eventType: string): SessionTurn["status"] => {
  ...
  if (eventType.includes("interrupt")) return "interrupted";
  if (eventType.includes("done") || eventType.includes("complete")) return "completed";
  if (eventType.includes("queue")) return "queued";
  if (eventType.includes("assistant") || eventType.includes("tool")) return "running";
  return "queued";
};
