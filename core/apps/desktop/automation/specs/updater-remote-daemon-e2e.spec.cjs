...
const REMOTE_CTX_BIN = String(process.env.CTX_AUTOMATION_REMOTE_CTX_BIN || "/tmp/ctx-e2e-bin/ctx").trim();
const REMOTE_CHANNEL = String(
  process.env.CTX_UPDATER_E2E_REMOTE_CHANNEL || process.env.RELEASE_CHANNEL || "stable",
).trim() || "stable";
const SSH_KEY_PATH = String(process.env.CTX_UPDATER_E2E_SSH_KEY_PATH || "").trim();
...
const remoteVersion = () => {
  const cmd = `if [ -x ${JSON.stringify(REMOTE_CTX_BIN)} ]; then ${JSON.stringify(REMOTE_CTX_BIN)} --version; else echo missing; fi`;
...
describe("updater remote daemon e2e", () => {
  ...
  it("connects over SSH, updates remote daemon, and remains connected", async () => {
    ...
    if (beforeVersion.toLowerCase() === "missing") {
      throw new Error(`remote ctx binary missing at ${REMOTE_CTX_BIN}`);
    }
