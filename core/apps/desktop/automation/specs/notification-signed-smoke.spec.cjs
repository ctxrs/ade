...
const waitForDeliveredNotification = async ({ deepLink, title, timeoutMs = 30000 }) => {
  ...
  await browser.waitUntil(async () => {
    ...
    timeout: timeoutMs,
    interval: 500,
    timeoutMsg: `delivered notification did not appear for title=${title}; last=${JSON.stringify(lastSnapshot)}`,
  });
  return matched;
};
...
describe("signed macOS notification smoke", () => {
  it("delivers a real system notification from a signed app bundle", async () => {
    ...
    if (!deepLink.includes(`task-${nonce}`)) {
      throw new Error(`notification automation snapshot missing expected deep link: ${JSON.stringify(automationSnapshot)}`);
    }
