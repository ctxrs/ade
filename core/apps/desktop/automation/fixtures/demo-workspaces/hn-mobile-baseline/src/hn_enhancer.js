import {
  appendDemoScenarioToLocalPath,
  HN_DEMO_SCENARIO_QUERY_KEY,
  readDemoScenarioIdFromSearch,
...
export function buildDemoLoginGoto(searchValue) {
  ...
  upstreamUrl.searchParams.set(HN_DEMO_SCENARIO_QUERY_KEY, readDemoScenarioIdFromSearch(searchValue));
...
export function buildFeedPreviewPath(searchValue) {
  return appendDemoScenarioToLocalPath(FEED_PREVIEW_PATH, readDemoScenarioIdFromSearch(searchValue));
}
...
function enhanceSettingsPage(doc) {
  ...
  if (form && settingsTable) {
    const insertionParent = settingsTable.tBodies[0] ?? settingsTable;
    const insertionRow = findSettingsInsertionRow(doc, settingsTable);
    if (insertionRow) {
      insertionParent.insertBefore(row, insertionRow);
    } else {
      insertionParent.append(row);
    }
