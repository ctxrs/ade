...
function createHackerNewsProxyMiddleware() {
  return async (request, response, next) => {
    ...
    try {
      ...
      const scenarioId = resolveRequestScenarioId(request.url);
      if (mockDemoProfilePage) {
        response.statusCode = 200;
        response.setHeader("cache-control", "no-store");
        response.setHeader("content-type", "text/html; charset=utf-8");
        response.end(rewriteHackerNewsHtml(buildDemoProfileHtml(scenarioId), { scenarioId }));
        return;
      }
      ...
      if (location) {
        response.setHeader("location", rewriteHackerNewsLocation(location, { scenarioId }));
      }
      ...
      if (contentType.includes("text/html")) {
        response.end(rewriteHackerNewsHtml(await upstream.text(), { mockDemoFrontPage, scenarioId }));
        return;
      }
