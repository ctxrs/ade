...
test("linux bundled launch smoke passes explicit tauri-driver native port", () => {
  ...
  assert.match(script, /CTX_LINUX_BUNDLED_LAUNCH_SESSION_TIMEOUT_MS/);
  assert.match(script, /120_000/);
  ...
  assert.match(remoteWorkspace, /CTX_REMOTE_WORKSPACE_DESKTOP_LAUNCH_SMOKE_SESSION_TIMEOUT_MS:-120000/);
  assert.match(remoteWorkspace, /CTX_AUTOMATION_APP_LAUNCH_LOG=\$smoke_dir\/app-launch\.log/);
  assert.match(fs.readFileSync(path.resolve(ROOT, "..", "..", "..", "scripts/tests/updater_remote_daemon_e2e.sh"), "utf8"), /CTX_UPDATER_REMOTE_E2E_CONTROLLER_LAUNCH_SMOKE_SESSION_TIMEOUT_MS:-120000/);
