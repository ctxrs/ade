...
test("launcher opens workspace wizard and does not expose remote ctx path input", async ({ page }) => {
  ...
  await page.getByTestId("wizard-remote-advanced-toggle").click();
  await expect(page.getByTestId("wizard-remote-port")).toBeVisible({ timeout: 20_000 });
  await expect(page.getByTestId("wizard-remote-data-dir")).toBeVisible({ timeout: 20_000 });
