...
const validBundle = (): E2EDiagnosticBundle => ({
  ...
  version: "0.65.131",
  test_name: "synthetic updater web e2e",
  ...
  trace_paths: ["traces/synthetic-updater.zip"],
...
test("web e2e diagnostic failure writer emits an attached schema-valid bundle", async ({}, testInfo) => {
  ...
  await fs.writeFile(testInfo.outputPath("traces/synthetic-updater.zip"), "fake-trace-bytes", "utf8");
...
test("web e2e diagnostic bundle builder fails closed for strict placeholder evidence", () => {
