...
const MARKDOWN_CORPUS: MarkdownSample[] = [
  {
    ...
    markdown:
      "I also verified the sample workspace is populated. Its active snapshot currently reports `7` active tasks, including `Sample Queue`.",
  },
  {
  ...
  },
  {
    ...
    markdown: "Compare `origin/main`, `Sample Queue`, and `ctx serve` before replying.",
  },
  {
  ...
  },
  {
    ...
    markdown:
      "This fixture is in a stable state now: the inline token path is green, the viewport row measurement check is green, and `verify:quick` is back to only the same synthetic sample failures. I’m using one short review pass now to reassess the next highest-value layout case after these two reductions, so the next move stays measurement-first instead of random fixture shaving.",
  },
  {
    name: "whitespace-code-comma-tail-long-prose",
    markdown:
      "The current local validation sample is concrete and fixable: `sample-runtime-helper` has a test helper that assumes Python `shutil.copytree(..., dirs_exist_ok=True)`, which is not available in the runner. I’m adjusting that first, because no other fixture change matters while the required check is red.",
  },
  {
    name: "whitespace-inline-code-prose-tail",
    markdown:
      "The `synthetic updater web e2e` fixture was recently wrapping at an awkward seam because the desktop harness examples were returning placeholder tokens and letting sample updater behavior enter the measurement path. I adjusted the immediate case, but that exposed test-fixture fragility.",
  },
  {
  ...
  },
  {
    ...
    markdown:
      "The layout example was in renderer.ts: every `PanelClosed` event called the single app-wide `MeasurementTracker.flush()`. That reset the shared measurement window for all panels, so the remaining panel recovered against a refreshed fixture and active rows got reconciled as pending, which is why examples looked delayed.",
  },
  {
...
const SOFT_BREAK_MARKDOWN_REGRESSIONS: SoftBreakMarkdownSample[] = [
  ...
  },
  {
    ...
    markdown: [
      ...
      "",
      "5. **Cleanup**",
      "   I’d add `tools/layout-fixture sweep`, but timed execution is a separate step. The tool can be ready now; wiring periodic sweeps can be a follow-up for another sample.",
      "",
      "6. **State location**",
      ...
      "",
      "7. **Run handle**",
      "   Generate a per-run handle and delete it on `down`. That avoids relying on shared mutable state and makes teardown easy to inspect.",
      "",
      "8. **Fixture level**",
      ...
      "",
      "My recommendation: implement the generic `experiments` pool with labels and TTL metadata now, no timed job yet. Then use it for the layout-load sample.",
    ].join("\n"),
  },
  {
...
const USER_MESSAGE_CORPUS = [
  ...
  },
  {
    ...
    params: {
      content: [
        "Keep `7` active tasks, including `Sample Queue`, aligned before replying.",
...
const ASSISTANT_CORPUS = [
  ...
  },
  {
    name: "three-chip-prose",
    params: {
      content: "Compare `origin/main`, `Sample Queue`, and `ctx serve` before replying.",
    },
  },
  {
    name: "period-after-code-tail",
