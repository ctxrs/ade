...
const INLINE_CODE_TAILS = [
  "release updater web e2e",
  "provider lifecycle smoke",
  "ctx-main drift guard",
...
const OPERATOR_LABELS = [
  "ctx-dev=true",
  "ctx-dev-pool=experiments",
  "ctx-dev-purpose=<purpose>",
  "ctx-dev-owner=<user>",
  "ctx-dev-expiry=<epoch>",
  "HETZNER_API_TOKEN",
  "core/.infisical.json",
  "scripts/dev_hetzner_machine.sh janitor",
  ".ctx/tmp/dev-machines/<run-id>.json",
  "/tmp/ctx-dev-machine-<run-id>.json",
  "--ttl-hours",
  "--keep",
] as const;
...
const SLASH_PROSE_TOKENS = [
  ...
  "Ubuntu/Debian",
  "VPC/security-group",
  "network-denied/runtime",
  "bootstrap/pending-authority",
...
const TABLE_CODES = ["m7i.4xlarge", "c7i.4xlarge", "CCX43", "ctx wrap rule", "observer.disconnect()"] as const;
...
function makeFamilyMarkdown(rng: SeededRandom, family: string): string {
  switch (family) {
    ...
    case "ordered_operator_list":
      return [
        `${words(rng, 5, 8)} before implementation:`,
        "",
        `1. **${rng.pick(BOLD_HEADS)}**`,
        `   ${words(rng, 5, 8)} \`${rng.pick(OPERATOR_LABELS)}\` or \`${rng.pick(OPERATOR_LABELS)}\` ${rng.pick(PROSE_TAILS)}`,
        `   \`${rng.pick(OPERATOR_LABELS)}\``,
        `   \`${rng.pick(OPERATOR_LABELS)}\``,
        "",
        `2. **${rng.pick(BOLD_HEADS)}**`,
        `   ${words(rng, 7, 10)} \`${rng.pick(OPERATOR_LABELS)}\`, ${words(rng, 5, 7)} \`${rng.pick(OPERATOR_LABELS)}\`.`,
      ].join("\n");
    default:
