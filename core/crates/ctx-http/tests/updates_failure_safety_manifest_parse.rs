...
#[tokio::test]
async fn updates_check_reports_route_build_identity_version() {
    let _env_lock = lock_env();
    ...
    let expected_current_version = current_update_version();
    assert_eq!(
        body.get("current_version").and_then(Value::as_str),
        Some(expected_current_version.as_str()),
        "updates check must use the route build identity version: {body}"
    );
}
