...
pub(crate) fn provider_status_is_usable(status: &ProviderStatus) -> bool {
    status.usability.usable
}

pub(crate) fn provider_status_unusable_reason(status: &ProviderStatus) -> Option<String> {
    status
        .usability
        .reason
        .clone()
...
pub(crate) fn apply_provider_usability_details(
    status: &mut ProviderStatus,
    data_root: &StdPath,
    managed: &installer::AgentServerConfigFile,
    matrix: &crate::provider_matrix::ProviderMatrix,
