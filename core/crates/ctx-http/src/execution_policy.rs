...
#[cfg(test)]
pub(crate) static EXECUTION_POLICY_TEST_ENV_LOCK: tokio::sync::Mutex<()> =
    tokio::sync::Mutex::const_new(());

#[derive(Debug, Clone, Copy, Eq, PartialEq)]
pub(crate) enum HostExecutionPolicy {
    AllowHost,
    SandboxOnly,
}

#[derive(Debug)]
pub(crate) struct ExecutionPolicyDenied {
    message: String,
}
...
pub(crate) fn is_execution_policy_denial(error: &anyhow::Error) -> bool {
    error
        .chain()
...
impl HostExecutionPolicy {
    ...
    fn from_env_value(raw: Option<&str>) -> anyhow::Result<Self> {
    ...
    pub(crate) fn validate_process_execution_mode_override(self) -> anyhow::Result<()> {
    ...
    pub(crate) fn normalize_loaded_execution_settings(self, settings: &mut ExecutionSettings) {
        if matches!(self, Self::SandboxOnly) && matches!(settings.mode, ExecutionMode::Host) {
    ...
    pub(crate) fn validate_install_target(self, target: InstallTarget) -> anyhow::Result<()> {
