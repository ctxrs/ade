...
async fn mobile_api_tokens_still_authorize_mobile_registration() {
    ...
    state
        ...
        .seed_mobile_api_profile_for_test(token, &["register_device"])
        .await
        .unwrap();
...
async fn mobile_api_tokens_without_device_registration_scope_reject_registration() {
    ...
    state
        .mobile_access_for_test()
        .seed_mobile_api_profile_for_test(token, &["secure_proxy"])
        .await
        .unwrap();
    ...
    assert_eq!(
        payload["error"],
        "mobile token is missing register_device scope"
    );
}
