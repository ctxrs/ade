...
pub(in crate::api::workspaces::management) async fn load_workspace_execution_config(
    ...
    let settings = workspaces.load_settings().await.map_err(|error| {
        (
    ...
    match workspaces
        .load_workspace_execution_override(ctx.workspace_id)
        .await
    {
...
pub(in crate::api::workspaces::management) async fn update_workspace_execution_config(
    ...
    let environment = parse_execution_environment(workspaces, req.environment.trim())?;
    ...
    workspaces
        .update_workspace_execution_config(
            ctx.workspace_id,
