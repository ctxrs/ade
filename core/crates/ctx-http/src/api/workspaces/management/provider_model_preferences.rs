...
fn parse_workspace_id(id: &str) -> Result<WorkspaceId, (StatusCode, Json<ApiErrorResp>)> {
...
) -> Result<ctx_core::models::Workspace, (StatusCode, Json<ApiErrorResp>)> {
    state
        .global_store()
...
async fn require_known_provider(
    ...
    .await;
    let known = crate::provider_matrix::get_entry(&matrix, provider_id).is_some()
        || state
            .providers
            .statuses
            .lock()
