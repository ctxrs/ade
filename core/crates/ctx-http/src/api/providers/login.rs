...
async fn resolve_cursor_login_runtime(state: &Arc<AppState>) -> anyhow::Result<std::path::PathBuf> {
    if let Some(runtime) =
        resolve_runtime_provider_command_from_config(&state.core.data_root, "cursor").await?
    {
        let path = std::path::PathBuf::from(runtime.command_abs_path);
...
async fn monitor_cursor_login(state: Arc<AppState>, login_id: String, label: Option<String>) {
    ...
    if let Err(err) = tokio::fs::write(&capture_path, b"").await {
        ...
        if let Some(entry) = map.get_mut(&login_id) {
            entry.status = "failed".to_string();
            entry.error = Some(format!("failed to initialize capture file: {err}"));
        }
        let _ = tokio::fs::remove_dir_all(&login_home).await;
        return;
    }
    ...
    let mut child = match cmd.spawn() {
        ...
        Err(err) => {
            ...
            if let Some(entry) = map.get_mut(&login_id) {
                entry.status = "failed".to_string();
                entry.error = Some(format!("failed to launch cursor-agent login: {err}"));
            }
            let _ = tokio::fs::remove_dir_all(&login_home).await;
            return;
        }
                        ...
                        if let Some(auth_token) = auth_token {
                            ...
                            .await
                            {
                                Ok(registry) => {
                                    ...
                                    restarts::restart_cursor_providers_for_auth_change(
                                        &state,
                                        "cursor auth updated",
                                    )
                                    .await;
