...
pub(super) async fn restart_gemini_providers_for_auth_change(state: &Arc<AppState>, reason: &str) {
    restart_provider_for_auth_change(state, "gemini", reason).await;
}

pub(super) async fn restart_qwen_providers_for_auth_change(state: &Arc<AppState>, reason: &str) {
    restart_provider_for_auth_change(state, "qwen", reason).await;
}

pub(super) async fn restart_amp_providers_for_auth_change(state: &Arc<AppState>, reason: &str) {
    restart_provider_for_auth_change(state, "amp", reason).await;
}

pub(super) async fn restart_mistral_providers_for_auth_change(state: &Arc<AppState>, reason: &str) {
    restart_provider_for_auth_change(state, "mistral", reason).await;
}

pub(super) async fn restart_kimi_providers_for_auth_change(state: &Arc<AppState>, reason: &str) {
    restart_provider_for_auth_change(state, "kimi", reason).await;
}

pub(super) async fn restart_copilot_providers_for_auth_change(state: &Arc<AppState>, reason: &str) {
    restart_provider_for_auth_change(state, "copilot", reason).await;
}

pub(super) async fn restart_cursor_providers_for_auth_change(state: &Arc<AppState>, reason: &str) {
