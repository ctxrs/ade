...
pub(super) async fn set_failed(state: &Arc<AppState>, login_id: &str, error: String) {
    crate::daemon::providers::set_mistral_login_failed(state, login_id, error).await;
}

pub(super) async fn set_failed_if_no_error(state: &Arc<AppState>, login_id: &str, error: String) {
    crate::daemon::providers::set_mistral_login_failed_if_no_error(state, login_id, error).await;
}

pub(super) async fn set_timeout_if_no_error(state: &Arc<AppState>, login_id: &str, error: String) {
    crate::daemon::providers::set_mistral_login_timeout_if_no_error(state, login_id, error).await;
}

pub(super) async fn set_auth_url(state: &Arc<AppState>, login_id: &str, auth_url: String) {
    crate::daemon::providers::set_mistral_login_auth_url(state, login_id, auth_url).await;
}
...
pub(super) async fn set_completion_status(
    ...
    crate::daemon::providers::finish_mistral_login_session(state, login_id, restart_result).await;
}
