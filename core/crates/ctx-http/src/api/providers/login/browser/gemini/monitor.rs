...
pub(super) async fn monitor_gemini_login(
    ...
    let paths = match setup::prepare_gemini_login_paths(&providers, &login_id).await {
        Ok(paths) => paths,
        Err(err) => {
            status::set_failed(&providers, &login_id, err).await;
            return;
        }
    };
    let provider_env = setup::gemini_provider_env(&providers, &paths.login_home);
    ...
    let auth_result = providers
        .authenticate_provider_session(
            ...
            ctx_provider_runtime::provider_session_auth::ProviderSessionAuthenticationRequest {
                ...
                env: provider_env,
                method_id: Some(providers.gemini_login_auth_method_id()),
                event_sink: event_tx,
    ...
    loop {
        let event_outcome = events::drain_gemini_login_events(
            &providers,
            &login_id,
            &mut event_rx,
            observed_auth_url,
        ...
        if completion::complete_gemini_login_if_credentials_exist(
            &providers, &login_id, &label, &paths,
        )
        .await
        {
            return;
