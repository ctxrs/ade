...
fn is_legacy_sandbox_candidate(
    ...
    Path::new(&worktree.root_path) == legacy_guest_worktree_root(worktree)
}
...
async fn normalize_legacy_worktree_root(
    ...
    store
        .update_worktree_root_path(worktree.id, &canonical_root_string)
        .await
        .with_context(|| {
            format!(
...
async fn backfill_worktree_binding(
    ...
    if host_sessions > 0 && Path::new(&worktree.root_path) == legacy_guest_worktree_root(worktree) {
        bail!(
            "legacy sandbox worktree {} uses a guest-root path but only host sessions; manual repair is required",
            worktree.id.0
        );
    }

    let repaired_worktree =
        normalize_legacy_worktree_root(state, store, workspace, worktree).await?;
...
pub(crate) async fn ensure_workspace_sandbox_bindings_backfilled(
    ...
    for worktree in worktrees {
        ...
        if !is_legacy_sandbox_candidate(&worktree, &sessions) {
            continue;
        }
        ...
        store
            ...
            .await
            .with_context(|| format!("saving sandbox binding for worktree {}", worktree.id.0))?;
...
"#
        ...
        std::fs::write(&sandbox_cli_path, sandbox_cli_script)
            .expect("write AVF sandbox helper shim");
...
"#,
    ...
    async fn lookup_workspace_store_backfills_legacy_sandbox_worktree_on_cold_open() {
        ...
        let (archive_url, archive_server) =
            spawn_static_http_server_with_suffix(archive_bytes.clone(), "guest-runtime.tar.gz")
                .await;
        ...
        let (egress_proxy_url, egress_proxy_server) =
            spawn_static_http_server_with_suffix(egress_proxy_bytes.clone(), "ctx-egress-proxy")
                .await;
        let _runtime_guard =
        ...
        let _avf_helper =
            EnvVarGuard::set("CTX_AVF_LINUX_HELPER_PATH", &helper_path.to_string_lossy());
