...
async fn get_workspace_active_snapshot(
    ...
    let store_snapshot = match (
        ...
        (Ok((snapshot_rev, archived_rev)), Ok((tasks, total_count))) => {
            Some(WorkspaceActiveSnapshot {
                workspace_id,
                snapshot_rev,
                archived_rev,
    ...
    let mut snapshot = match store_snapshot {
        Some(snapshot) => snapshot,
        None => {
            if let Some(cached) = state.cached_workspace_active_snapshot(workspace_id).await {
                if !cached.active.tasks.is_empty() {
                    return Ok(Json(cached));
                }
            }
            let hub_snapshot = state
                .workspace_active_snapshot
                .active_snapshot(workspace_id, limit)
                .await;
            if !hub_snapshot.active.tasks.is_empty() {
                return Ok(Json(hub_snapshot));
            }
            if let Some(cached) = state.cached_workspace_active_snapshot(workspace_id).await {
                return Ok(Json(cached));
            }
    ...
    if snapshot.active.total_count == 0 {
        let hub_snapshot = state
            .workspace_active_snapshot
            .active_snapshot(workspace_id, limit)
            .await;
...
async fn get_workspace_active_heads(
    ...
    let store_batch = match (
        ...
        (Ok((snapshot_rev, _)), Ok(heads)) => Some(WorkspaceActiveHeadBatch {
            workspace_id,
            snapshot_rev,
            heads,
        }),
    ...
    let mut batch = match store_batch {
        Some(batch) => batch,
        None => {
            if let Some(cached) = state.cached_workspace_active_heads(workspace_id).await {
                if !cached.heads.is_empty() {
                    return Ok(Json(cached));
                }
            }
            let hub_batch = state
                ...
                .active_heads(workspace_id)
                .await;
            if !hub_batch.heads.is_empty() {
                return Ok(Json(hub_batch));
            }
            if let Some(cached) = state.cached_workspace_active_heads(workspace_id).await {
                return Ok(Json(cached));
            }
    ...
    if batch.heads.is_empty() {
        if let Ok(session_ids) = store.list_workspace_active_session_ids(workspace_id).await {
    ...
    state.cache_workspace_active_heads(batch.clone()).await;
    Ok(Json(batch))
}
