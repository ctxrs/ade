...
pub async fn apply_matrix_to_status(
    ...
    let context_version = current_ctx_version.and_then(updates::normalize_version_str);
    ...
    let is_archive_install = install_target_from_status(status)
        .and_then(|target| {
            entry
                ...
                .and_then(|install| {
                    install
                        .archive_target(crate::installer::resolve_matrix_target_key(target).ok()?)
                })
                .map(|_| true)
        })
        .unwrap_or(matches!(
...
pub(super) async fn detect_managed_artifact_fingerprint_mismatch(
    ...
    let archive_target_present = entry
        ...
        .and_then(|install| {
            install
                .archive_target(crate::installer::resolve_matrix_target_key(requested_target).ok()?)
        })
        .is_some();
