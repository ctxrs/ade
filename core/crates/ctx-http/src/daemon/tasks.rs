...
impl TasksHandle {
    ...
    pub(crate) async fn unarchive_task(&self, task_id: TaskId) -> Result<Task, TaskLifecycleError> {
        let Some((store, task, workspace)) = self.load_task_context(task_id).await? else {
            return Err(TaskLifecycleError::NotFound);
        };
    ...
    pub(crate) async fn delete_task(&self, task_id: TaskId) -> Result<(), TaskLifecycleError> {
        let Some((store, task, workspace)) = self.load_task_context(task_id).await? else {
            return Err(TaskLifecycleError::NotFound);
        };
