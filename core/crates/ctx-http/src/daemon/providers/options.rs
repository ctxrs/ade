...
#[derive(Debug)]
pub(crate) enum EffectivePreferredModelError {
...
async fn effective_model_payload_for_workspace(
    ...
    let launch_config = super::load_provider_launch_config_snapshot(state, provider_id).await;
    if launch_config.managed_config_error.is_some() {
        return Ok(None);
    }

    let install_target = super::install_target_for_workspace(state, workspace.id)
        .await
    ...
    let cache = super::options_cache::ProviderOptionsCacheSnapshot::load(
        state,
        workspace.id,
        install_target,
        provider_id,
    ...
    let active_auth = if launch_config.source_config_error.is_none() {
        super::provider_has_active_auth_for_workspace_runtime(
            state,
            workspace,
            provider_id,
            source_config,
    ...
    if let Some(has_active_auth) = active_auth {
        ...
        {
            let probe =
                super::probe_runtime_models_for_provider_options(state, workspace, provider_id)
                    .await
                    .map_err(EffectivePreferredModelError::ExecutionSettings)?;
