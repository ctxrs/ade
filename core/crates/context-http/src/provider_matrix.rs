...
pub fn default_matrix_base_url() -> String {
    std::env::var("CONTEXT_PROVIDER_MATRIX_BASE_URL")
        .unwrap_or_else(|_| updates::default_download_base_url())
}

pub fn default_matrix_channel() -> String {
    std::env::var("CONTEXT_PROVIDER_MATRIX_CHANNEL").unwrap_or_else(|_| "stable".to_string())
}
            ...
            match release_for_version(entry, version) {
                Some(release) => {
                    if release.status != ProviderReleaseStatus::Supported {
                        ...
                        supported = false;
                        let mut msg = "Provider version requires a newer Context build".to_string();
                        if let Some(min) = release.context_min.as_ref() {
                            msg = format!("Provider version requires Context >= {min}");
                        }
...
async fn detect_provider_version(
    ...
    cfg: &AgentServerConfigFile,
    entry: &ProviderMatrixEntry,
    status: &context_providers::adapters::ProviderStatus,
) -> Option<String> {
    if !status.installed {
        return None;
    }
