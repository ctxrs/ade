...
const ATTACHMENTS_CONFIG_PATH: &str = ".context/attachments.toml";
...
async fn ensure_git_exclude(worktree_root: &Path) -> Result<()> {
    ...
    let lines = [".context/.refs/", ".context/.docs/"];
...
fn default_mount_relpath(kind: &WorkspaceAttachmentKind, name: &str) -> String {
    ...
    match kind {
        WorkspaceAttachmentKind::ReferenceRepo => format!(".context/.refs/{safe_name}"),
        WorkspaceAttachmentKind::DocMirror => format!(".context/.docs/{safe_name}"),
    }
}
