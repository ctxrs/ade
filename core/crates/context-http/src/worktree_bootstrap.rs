...
use context_core::models::{Workspace, Worktree, WorktreeBootstrapNotice, WorktreeBootstrapStatus};
use context_store::WorktreeBootstrapResultUpdate;

use crate::daemon::AppState;
use crate::logs;

const CONFIG_REL_PATH: &str = ".context/config.toml";
...
async fn prepare_worktree_bootstrap(
    ...
    let bootstrap = match load_bootstrap_config(worktree, workspace).await {
        ...
        Err(err) => {
            ...
            log.push_str("# Context worktree bootstrap\n");
...
async fn emit_failure_notice(
    state: &AppState,
    workspace_id: context_core::ids::WorkspaceId,
