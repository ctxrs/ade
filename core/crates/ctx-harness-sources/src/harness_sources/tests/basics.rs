...
fn ctx_managed_relay_url_matching_respects_origin_and_path_boundaries() {
    std::env::set_var(
        "CTX_MANAGED_RELAY_BASE_URL",
        "https://relay.example.invalid/relay",
    );
    let prefix = Url::parse("https://relay.example.invalid/relay").expect("prefix url");
    let matching_child =
        Url::parse("https://relay.example.invalid/relay/openai/v1").expect("matching child url");
    let matching_exact =
        Url::parse("https://relay.example.invalid/relay").expect("matching exact url");
    let bad_host = Url::parse("https://relay.example.invalid.evil.example/relay/openai/v1")
        .expect("bad host url");
    let bad_path =
        Url::parse("https://relay.example.invalid/relayx/openai/v1").expect("bad path url");
    ...
    assert!(!base_url_matches_ctx_managed_prefix(
        &matching_child,
        &Url::parse("https://relay.example.invalid").expect("origin-only prefix url")
    ));
    assert!(base_url_uses_ctx_managed_relay(
        "https://relay.example.invalid/relay/openai/v1"
    ));
    assert!(!base_url_uses_ctx_managed_relay(
        "https://relay.example.invalid/functions/v1/openai"
    ));
    assert!(!base_url_uses_ctx_managed_relay(
        "https://relay.example.invalid/relayx/openai/v1"
    ));
}
