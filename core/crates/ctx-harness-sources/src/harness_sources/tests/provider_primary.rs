...
async fn codex_ctx_managed_relay_endpoint_keeps_provider_id_and_marks_runtime_backend() {
    ...
    let endpoint = upsert_provider_endpoint(
        ...
        HarnessEndpointUpsert {
            ...
            base_url: Some("https://relay.example.invalid/relay/openai/v1".to_string()),
            api_shape: Some(HarnessApiShape::OpenaiResponses),
            auth_type: None,
    ...
    assert_eq!(
        resolved
            ...
            .map(String::as_str),
        Some("https://relay.example.invalid/relay/openai/v1")
    );
    assert_eq!(
        resolved
            .env
    ...
    let endpoint = upsert_provider_endpoint(
        ...
        HarnessEndpointUpsert {
            ...
            service_account_json: Some(
                r#"{"type":"service_account","project_id":"vertex-project","private_key_id":"key-id","private_key":"not-a-real-private-key","client_email":"ctx-vertex@test.iam.gserviceaccount.com","client_id":"1234567890"}"#
                    .to_string(),
            ),
            project_id: None,
            location: None,
