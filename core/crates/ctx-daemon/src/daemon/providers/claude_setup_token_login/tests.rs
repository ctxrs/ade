...
use super::auth_url::{
    auth_url_looks_complete, extract_auth_url, extract_claude_setup_token,
    extract_preferred_claude_auth_url, normalize_claude_login_line,
    read_claude_browser_open_capture_url, read_trailing_claude_login_lines,
...
#[test]
fn extract_claude_setup_token_accepts_wrapped_prefix() {
    let output = concat!(
    ...
    assert_eq!(
        extract_claude_setup_token(output).as_deref(),
        Some(concat!("sk-ant-", "oat-test-token"))
    );
}

#[test]
fn extract_claude_setup_token_rejects_prefix_without_token_body() {
    assert_eq!(extract_claude_setup_token("sk-ant-oat\n"), None);
}
...
async fn read_trailing_claude_login_lines_waits_for_late_arrival() {
    ...
    std::thread::spawn(move || {
        ...
        let _ = tx.send(format!("{}{}", "sk-ant-", "oat-late-token"));
    });
    let lines = read_trailing_claude_login_lines(&mut rx, Duration::from_secs(2)).await;
    assert_eq!(lines, vec![format!("{}{}", "sk-ant-", "oat-late-token")]);
}
