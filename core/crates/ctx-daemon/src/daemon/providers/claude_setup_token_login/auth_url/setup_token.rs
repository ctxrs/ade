...
const CLAUDE_SETUP_TOKEN_PREFIX: &str = "sk-ant-oat";
const CLAUDE_SETUP_TOKEN_PREFIX_HEAD: &str = "sk-ant-";
...
pub(in crate::daemon::providers::claude_setup_token_login) fn extract_claude_setup_token(
    ...
    for (idx, line) in lines.iter().enumerate() {
        let Some(start) = line.find(CLAUDE_SETUP_TOKEN_PREFIX_HEAD) else {
            continue;
        };
        ...
        if cleaned.starts_with(CLAUDE_SETUP_TOKEN_PREFIX)
            && cleaned.len() > CLAUDE_SETUP_TOKEN_PREFIX.len()
        {
            return Some(cleaned);
        }
    }
