...
pub async fn start_amp_browser_login(
    ...
    let session = login_sessions::start_amp_login_session(state).await;
    let login_id = session.login_id.clone();
    let state = Arc::clone(state);
    tokio::spawn(async move {
        monitor_amp_login(state, login_id, label).await;
    });
    session
}

async fn monitor_amp_login(state: Arc<DaemonState>, login_id: String, label: Option<String>) {
    let paths = match accounts::prepare_amp_login_paths(&state, &login_id).await {
        Ok(paths) => paths,
        Err(error) => {
            login_sessions::set_amp_login_failed(&state, &login_id, error).await;
            return;
        }
    };
    let provider_env = accounts::amp_login_provider_env(&state, &paths.amp_home);
    ...
    .await
    {
        ...
        Err(err) => {
            login_sessions::set_amp_login_failed(&state, &login_id, err).await;
    ...
    loop {
        ...
        match next_amp_login_event(&state, &login_id, &mut event_rx, &mut progress).await {
            ...
            AmpLoginEventOutcome::Success => {
                complete_amp_login(&state, &login_id, &label, progress.observed_email.clone())
                    .await;
                common::cleanup_login_home(&paths.login_home).await;
                return;
            }
...
async fn next_amp_login_event(
    ...
    if let Some(auth_url) = common::extract_auth_url_from_value(&event.payload_json) {
        ...
        login_sessions::set_amp_login_auth_url(state, login_id, auth_url).await;
    }
    ...
    if common::is_auth_failure_notice_code(common::auth_notice_code(&event.payload_json)) {
        ...
        login_sessions::set_amp_login_failed(state, login_id, message).await;
...
async fn complete_amp_login(
    ...
    match accounts::upsert_amp_account_for_login(state, label.clone(), observed_email).await {
        Ok(outcome) => {
            let (_, restart_result) = outcome.into_restart_result();
            login_sessions::finish_amp_login_session(state, login_id, restart_result).await;
        }
        Err(err) => {
