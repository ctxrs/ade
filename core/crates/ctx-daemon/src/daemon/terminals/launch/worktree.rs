...
use super::{internal_error, not_found, TerminalLaunchError, TerminalLaunchHost};
...
pub(super) async fn resolve_terminal_worktree(
    ...
    if let Some(worktree_id) = worktree_id {
        return load_explicit_worktree(host, workspace_id, worktree_id).await;
    }
...
pub async fn infer_terminal_worktree(
    ...
    if let Ok(store) = host.store_for_workspace(workspace_id).await {
