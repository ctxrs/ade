...
mod tests {
    ...
    fn normalizer_removes_paths_provider_refs_pty_streams_and_secrets() {
        let payload = json!({
            "path": "/Users/example-user/src/ctx/file.rs",
            ...
            "nested": {
                "message": "open /tmp/project/.env with ghp_testplaceholder123456789012"
            },
            "token_usage": {
                "input": 12
            },
    ...
    fn transcript_text_normalization_redacts_inline_paths_and_tokens() {
        let normalized = normalize_archive_text(
            "Read /home/example-user/project/main.rs then used sk-test-placeholder-1234567890.",
        );
        assert_eq!(
            normalized.text,
    ...
    fn summary_scope_omits_payloads() {
        let event = SessionEvent {
            ...
            payload_json: json!({"path": "/Users/example-user/src/ctx/file.rs"}),
            transient: false,
            created_at: Utc.with_ymd_and_hms(2026, 4, 28, 12, 0, 0).unwrap(),
        };
