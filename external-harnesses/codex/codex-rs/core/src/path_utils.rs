...
mod tests {
    ...
    mod wsl {
        ...
        fn wsl_mnt_drive_paths_lowercase() {
            let normalized = normalize_for_wsl_with_flag(PathBuf::from("/mnt/C/Users/example-user"), true);

            assert_eq!(normalized, PathBuf::from("/mnt/c/users/example-user"));
        }

        #[test]
        fn wsl_non_drive_paths_unchanged() {
            let path = PathBuf::from("/mnt/cc/Users/example-user");
        ...
        fn wsl_non_mnt_paths_unchanged() {
            let path = PathBuf::from("/home/example-user");
            let normalized = normalize_for_wsl_with_flag(path.clone(), true);
