...
That means first-class task and worktree management. Diff-first review. Artifact capture for things like UI screenshots. Durable transcripts and attribution. Verification loops. Bounded autonomy inside containers and network policy. Vendor-neutral harness and model choice. A coordination layer for many agents, not just one.
...
The capability wave is already here. What is still missing is the interface, the review loop, the trust model, and the coordination layer that let teams use that capability professionally.
