...
codex_crp_export_cache_env "${ROOT_DIR}" "codex-crp-test"
...
if [[ "${PROFILE}" == "release" ]]; then
  codex_crp_run_rust "${ROOT_DIR}" test -q --manifest-path "${WORKSPACE_MANIFEST}" --release -p codex-crp
else
  codex_crp_run_rust "${ROOT_DIR}" test -q --manifest-path "${WORKSPACE_MANIFEST}" -p codex-crp
fi
